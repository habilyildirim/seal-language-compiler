...
void overflow_control(const unsigned int c, PARSER_LAYER_ERROR_TYPE err)
{
	if (c >= tokens_counter)
	{
		parser_error(sources_files[0], tokens[c - 1].line, 
						tokens[c - 1].column, err, NULL);	
	}
}
...
EXPR* parse_primary(unsigned int *i)
{
    ...
    if (tok.token_group == _IDENTIFIER) 
    {
    	...
    	if (*i < tokens_counter && tokens[*i].token_type == SYMBOL_LPAREN)
    	{
        	...
        	if (tokens[*i].token_type != SYMBOL_RPAREN)
        		parser_error(sources_files[0], tokens[*i - 1].line, tokens[*i - 1].column, WRONG_EXPRESSION, NULL);
    ...
    parser_error(sources_files[0], tok.line, tok.column, WRONG_EXPRESSION, NULL);
}
...
AST parse_include(unsigned int *i, unsigned int c)
{
	...
	{
		printf(tokens[*i + 1].value);
		parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_INCLUDE, NULL);
	}
...
AST parse_macro(unsigned int *i, unsigned int c)
{
	...
	if (tokens[*i].token_type != IDENTIFIER)
		parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_MACRO, NULL);
...
AST parse_var(unsigned int *i, unsigned int is_unsigned, unsigned int c)
{
	...
	if (is_unsigned)
	{
		...
		if (tokens[*i].token_group != DTYPE)
			parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_UVAR, NULL);
	...
	if (tokens[*i].token_type != IDENTIFIER)
		parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_VAR, NULL);
...
AST parse_jumper(unsigned int *i, unsigned int c)
{
	...
	if (tokens[*i].token_type != SYMBOL_LPAREN)
		parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_JUMPER, NULL);
...
AST parse_label(unsigned int *i, unsigned int c)
{
	...
	if (tokens[*i].token_type != IDENTIFIER)
		parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_LABEL, NULL);
...
AST parse_function(unsigned int *i, unsigned int c)
{
	...
	if (tokens[*i].token_group != DTYPE)
		parser_error(sources_files[0], tokens[*i].line, tokens[*i].column, UNEXPECTED_FUNCTION, NULL);
	...
	while (tokens[*i].token_type != SYMBOL_RPAREN)
	{
		...
		if (tokens[*i].token_group == DTYPE)
		{	
			result.function.args = realloc(result.function.args, sizeof(*result.function.args) * (argc + 1));
	...
	for (unsigned int i = 0; i < tokens_counter; i++)
	{
		...
		switch (tokens[i].token_type)
		{
			...
			case KEYWORD_LABEL:
				...
				break;
			default:
				parser_error(sources_files[0], tokens[i].line, tokens[i].column, UNEXPECTED, NULL);
		}
